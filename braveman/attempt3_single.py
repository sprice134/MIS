...
def greedy_mis_set(H):
    ...
    and removes it and its neighbors.
    """
    independent_set = set()
    H_copy = H.copy()
...
def main():
    ...
    print(f"\nMaximum degree (Δ) of the graph: {max_degree}")
    ...
    print("\n3) Computed noisy degree for each vertex:")
    for v in sorted(deg_gI):
        print(f"  Vertex {v}: {deg_gI[v]}")

    # Step 4: Define set L = { v in V : deg(v) <= Δ/2 }.
    L = {v for v in G.nodes() if G.degree(v) <= max_degree / 2}
    print("\n4) Set L (vertices with deg(v) <= Δ/2):")
    print("  ", sorted(L))
    ...
    for v in G.nodes():
        if v not in L:
            d = G.degree(v)
            s_v = (0.5 - args.epsilon) * d + 6 * math.sqrt(math.log(max_degree)) * (0.5 - args.epsilon) * math.sqrt(d)
            s_values[v] = s_v
            if deg_gI[v] <= s_v:
                S.add(v)
    print("\n5) For vertices not in L, threshold s_v and noisy degree:")
    for v in sorted(s_values):
        print(f"  Vertex {v}: s_v = {s_values[v]:.4f}, noisy degree = {deg_gI[v]}")
    print("\nSet S (vertices in V\\L with noisy degree <= s_v):")
    print("  ", sorted(S))
    ...
    print("\n6) Greedy MIS on G[S ∪ L]:")
    print("  Greedy MIS =", sorted(mis_result))
    print("  True MIS   =", sorted(mis_star))

if __name__ == "__main__":
    main()
    '''
    python attempt3_single.py \
    ...
    python attempt3_single.py \
        --graph_file ../modelAttempt2_5/test_generated_graphs/nodes_15/removal_75percent/graph_21.edgelist \
        --json_file ../modelAttempt2_5/test_mis_results_grouped_v3/nodes_15_removal_75percent.json \
        --epsilon 0.20
    '''
