...
def main():
    ...
    print("\n4) Set L (vertices with deg(v) <= 36 ln(n)):")
    print("  ", sorted(L))
    ...
    for v in G.nodes():
        if v not in L:
            d = G.degree(v)
            s_v = (0.5 - args.epsilon) * d + 6 * math.sqrt(math.log(n)) * math.sqrt(d)
            s_values[v] = s_v
            if deg_gI[v] <= s_v:
                S.add(v)
    print("\n5) For vertices not in L, threshold $s_v$ and $\\widetilde{{deg}}_{{I^*}}(v)$:")
    for v in sorted(s_values):
        print(f"  Vertex {v}: $s_v$ = {s_values[v]:.4f}, $\\widetilde{{deg}}_{{I^*}}(v)$ = {deg_gI[v]}")
